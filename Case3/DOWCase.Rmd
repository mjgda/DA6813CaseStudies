...
DOWdata$next_week_change<-DOWdata$next_weeks_close-DOWdata$next_weeks_open
...
```{r}
model<-glm(percent_change_next_weeks_price~time+time:percent_return_next_dividend+close,family="gaussian",data=train)
summary(model)
plot(model)
```

```{r}
model<-glm(percent_change_next_weeks_price~percent_change_price+time+open+percent_return_next_dividend+close+high+low,family="gaussian",data=train)
summary(model)
...
trainmse<-mean(modsum$deviance.resid^2)
testmse<-mean((prediction-target$percent_change_next_weeks_price)^2)
print(trainmse)
print(testmse)

plot(target$time,target$percent_change_next_weeks_price,main=i)
