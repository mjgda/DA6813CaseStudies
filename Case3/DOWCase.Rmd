...
```
```{r}
length(names(DOWdata))
for(i in 4:length(names(DOWdata))){
  nam=names(DOWdata)[i]
  print(paste(nam,"lag",sep="_"))
  DOWdata=DOWdata %>%
          group_by(stock)%>%
          mutate(paste(nam,"lag",sep="_")=dplyr::lag(nam,n=1))
}
```
...
metrics=matrix(c("TrainMSE","TestMSE"))
...
metrics=matrix(c("TrainMSE","TestMSE"))
metrics_val=matrix(c(trainmse,testmse))
