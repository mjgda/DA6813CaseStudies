...
bbtrain<-upSample(x=bbtrain[,-ncol(bbtrain)],y=bbtrain$Choice)

#lose extraneous class variable from upsampling
bbtrain<-bbtrain[-11]

bbtrain$Choice <- factor(bbtrain$Choice, levels=rev(levels(bbtrain$Choice)))
...
tunedsvm=tune(svm,Choice~.,data=bbtrain,kernel ="linear",ranges=list(cost=c(0.001, 0.01, 0.1, 1,5,10,100)))
...
They have specified that they want to find the most predictively useful model out of the following options: logistic regression, linear regression, and support vector machine.
